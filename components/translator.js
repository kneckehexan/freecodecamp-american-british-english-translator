...
class Translator {
  ...
  expressions(language, text){
    ...
    var match, indexPairs = [], newExpressions = [];
    var i = 0;
    var lanObj = language === 'british-to-american' ? britishOnly: americanOnly;
   ...
   */
  words(language, text){
    var lanObj = language === 'british-to-american' ? this.flipObj(americanToBritishSpelling): americanToBritishSpelling;
    var match, textArrayWord = [text.sentence], newWords = [];
    var i = 0;
    Object.entries(lanObj).forEach(([key, val]) => {
      let reWord = '\\b' + key + '\\b';
  ...
  titles(language, text){
    var lanObj = language === 'british-to-american' ? this.flipObj(americanToBritishTitles): americanToBritishTitles;
    ...
    Object.entries(lanObj).forEach(([key, val]) => {
      ...
      if(language === 'american-to-british'){
        regex = new RegExp('\\b' + this.escapeRegExp(key).split('\\')[0] + '(?:\\.|||\\b)', 'gi');
      }
   ...
   */
  time(language, text){
    var pattern = language === 'british-to-american' ? '(0?[1-9]|1[012])(\.)([0-5][0-9])' : '(0?[1-9]|1[012])(:)([0-5][0-9])';
    var charReplacement = language === 'british-to-american' ? ':':'.';
    var regex = new RegExp(pattern, 'g');
    var match, newTime = [];
    if(match = text['sentence'].match(regex)){
      match.forEach((m) => newTime.push(m.replace(regex, '$1' + charReplacement + '$3')));
//      newTime.push(...match);
    }
    const translatedTime = text['sentence'].replace(regex, '$1' + charReplacement + '$3');
    const translation = {
  ...
  highlighter(newText, newParts, language){
    ...
    for(let i = 0; i < newParts.length; i++){
      ...
      if(language === 'american-to-british'){
        regex = new RegExp('\\b' + this.escapeRegExp(newParts[i]).split('\\')[0] + '(?:\\.|||\\b)', 'gi');
      }
      if(newParts[i].match(/(0?[1-9]|1[012])(\.)([0-5][0-9])/)){
        regex = new RegExp('\\b' + this.escapeRegExp(newParts[i]) + '\\b', 'g');
        highlightedText[i+1] = highlightedText[i].replace(regex, '<span class="highlight">' + newParts[i] + '</span>');
      } else if(newParts[i].match(/(0?[1-9]|1[012])(\:)([0-5][0-9])/)){
        regex = new RegExp('\\b' + newParts[i] + '\\b', 'g');
        highlightedText[i+1] = highlightedText[i].replace(regex, '<span class="highlight">' + newParts[i] + '</span>');
      } else {
        highlightedText[i+1] = highlightedText[i].replace(regex, '<span class="highlight">' + this.escapeRegExp(newParts[i]).split('\\')[0] + '</span>');
      }
    }
